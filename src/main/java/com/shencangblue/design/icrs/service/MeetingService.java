...
public class MeetingService {
    ...
    @Transactional
    public Iterable<Meeting> findAllByStartTimeAfterAndEndTimeBefore(Timestamp nowTime) {
//        return meetingDao.findAllByStartTimeAfterAndEndTimeBefore(newTime,nowTime1);
        ...
        for (Meeting meeting : meetings) {
            if (meeting.getStartTime().compareTo(nowTime) < 0 && meeting.getEndTime().compareTo(nowTime) > 0) {
                res.add(meeting);
            }
        }
        return res;
